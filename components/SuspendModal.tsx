...
export default function SuspendModal({
  ...
  return (
    <ModalShell z="z-[600]" padding="p-8 sm:p-12">
      ...
      <p className="text-zinc-500 mb-8 text-[10px] uppercase tracking-widest leading-relaxed">Sperren bis einschließlich</p>
          ...
          return (
            <button
              ...
              className={`py-3 rounded-2xl border text-[9px] font-black uppercase tracking-widest active:scale-95 transition-all ${
                until === date ? "border-red-500 text-red-500 bg-red-500/10" : "border-zinc-800 text-zinc-400 bg-black"
              }`}
            >
              {p.label}
            </button>
      ...
      <input
        ...
        className="w-full p-4 bg-black border border-zinc-800 rounded-2xl outline-none focus:border-red-500/50 text-red-500 font-black transition-all [color-scheme:dark] mb-3"
      />
      <input
        type="text"
        value={reason}
        ...
        maxLength={200}
        className="w-full p-4 bg-black border border-zinc-800 rounded-2xl outline-none focus:border-red-500/50 text-white font-bold transition-all placeholder:text-zinc-700 mb-3"
      />
      <p className="text-[9px] text-zinc-600 mb-8 leading-relaxed uppercase font-bold italic">
        Bis {until ? formatDate(until, { weekday: "short", day: "2-digit", month: "long" }) : "?"} · offene Buchungen werden entfernt
      </p>

      <div className="flex gap-4">
        <button onClick={onClose} className="flex-1 py-4 bg-zinc-800 text-white rounded-2xl font-black uppercase text-[10px] active:scale-95 transition-all">
          Abbrechen
        </button>
        <button
          onClick={submit}
