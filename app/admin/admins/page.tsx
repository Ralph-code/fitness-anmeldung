...
export default function AdminsPage() {
  ...
  const handleConfirm = () =>
    run(async () => {
      ...
      if (confirm.kind === "reset") {
        ...
        showToast("Admin gelöscht", "error");
      }
      setConfirm(null);
      await load();
    });
  ...
  return (
    <AdminPage title="Admins" subtitle="Superadmin-Bereich" superAdminOnly>
      {/* Neuer Admin */}
      <div className={`${cardClass} mb-6`} data-tour="admins-new">
              ...
              <div key={a.uid} className={`p-5 sm:p-6 rounded-[2rem] sm:rounded-[2.5rem] border transition-all ${a.isSuperAdmin ? "border-[var(--accent-30)] bg-[var(--accent-05)]" : "border-[var(--border-soft)] bg-[var(--surface-soft)]"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[var(--text-dim)] text-[10px] uppercase font-bold tracking-widest mt-1 truncate">
                      {a.username} · Login: {formatDay(a.lastSignIn)}
                    </p>
                  </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {a.isSuperAdmin ? (
                    ...
                    <>
                      <button onClick={() => setConfirm({ kind: "reset", admin: a })} className={`${PILL} border-[var(--border)] text-[var(--text-muted)]`}>Passwort neu</button>
                      <button onClick={() => setConfirm({ kind: "delete", admin: a })} className={`${PILL} border-red-500/20 text-red-500`}>Löschen</button>
                    </>
                  )}
                </div>
              </div>
