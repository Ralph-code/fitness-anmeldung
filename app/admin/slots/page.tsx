...
const timeClass = "w-full min-w-0 bg-[var(--bg)] border border-[var(--border)] rounded-2xl px-2 py-3 sm:p-4 text-lg sm:text-2xl text-center font-black italic tracking-tighter text-[var(--text)] outline-none focus:border-[var(--accent)] transition-all ";
...
export default function SlotsAdmin() {
  ...
  const save = async () => {
    ...
    try {
      ...
      showToast(res.removed ? `Gespeichert · ${res.removed} Buchungen storniert` : "Gespeichert");
    } catch (e) {
  ...
  return (
    <AdminPage title="Slots" subtitle="Zeitplan & Plätze" ready={ready} className="pb-44">
      {/* Buchungsfenster */}
      <div className={`${cardClass} mb-6`} data-tour="slots-opens">
        <div className="relative z-10">
          ...
          <input
            ...
            className="w-full min-h-[4.5rem] bg-[var(--bg)] border border-[var(--border)] rounded-2xl p-4 sm:p-5 text-3xl font-black text-[var(--accent-text)] outline-none focus:border-[var(--accent-50)] transition-all shadow-inner"
          />
      ...
      <div className="space-y-4">
        {current.slots.map((slot, i) => (
          <div key={slot.id} data-tour={i === 0 ? "slots-slot" : undefined} className="p-4 sm:p-6 rounded-[2rem] sm:rounded-[2.5rem] border border-[var(--border-soft)] bg-[var(--surface-soft)]">
      ...
      {/* Speichern-Leiste */}
      <div data-tour="slots-save" className="fixed bottom-0 inset-x-0 z-[400] bg-[var(--overlay)] backdrop-blur-xl border-t border-[var(--border)] p-4">
      ...
      <Toast toast={toast} position="bottom-32" />
      <Tour id="slots" />
    </AdminPage>
  );
}
